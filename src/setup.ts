...
async function updateWaybarConfig(): Promise<boolean> {
  ...
  try {
    if (!existsSync(WAYBAR_CONFIG_FILE)) {
      spinner.stop(colorize('Waybar config not found at ' + WAYBAR_CONFIG_FILE, semantic.danger));
      return false;
    }
    ...
    if (content.includes('custom/qbar-claude')) {
      spinner.stop(colorize('qbar modules already in config (skipped)', semantic.subtitle));
      return true;
    }
    ...
    if (lastBrace === -1) {
      spinner.stop(colorize('Invalid JSON structure in config', semantic.danger));
      return false;
    }
    ...
    spinner.stop(colorize(`Config updated (backup: ${backupPath})`, semantic.good));
    return true;
  } catch (error) {
    spinner.stop(colorize(`Failed to update config: ${error}`, semantic.danger));
    return false;
  }
}
...
async function updateWaybarStyles(): Promise<boolean> {
  ...
  try {
    ...
    // Check if qbar styles already exist
    if (content.includes('#custom-qbar-claude')) {
      spinner.stop(colorize('qbar styles already in CSS (skipped)', semantic.subtitle));
      return true;
    }
    ...
    spinner.stop(colorize(`Styles added (backup: ${backupPath})`, semantic.good));
    return true;
  } catch (error) {
    spinner.stop(colorize(`Failed to update styles: ${error}`, semantic.danger));
    return false;
  }
}
...
async function createSymlink(): Promise<boolean> {
  ...
  try {
    ...
    spinner.stop(colorize(`Failed to create symlink: ${error}`, semantic.danger));
    return false;
  }
}
...
async function reloadWaybar(): Promise<void> {
  ...
  try {
    ...
    spinner.stop(colorize('Could not reload Waybar (try manually)', semantic.warning));
  }
}
...
async function main() {
  ...
  if (allOk) {
    p.outro(colorize('Setup complete! Use "qbar menu" to get started.', semantic.good));
  } else {
    p.outro(colorize('Setup finished with warnings. Check messages above.', semantic.warning));
  }
}
